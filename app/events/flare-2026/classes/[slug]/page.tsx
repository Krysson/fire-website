...
export async function generateStaticParams() {
  const classes = getClasses('flare-2026');
  return classes.map((c) => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: ClassPageProps): Promise<Metadata> {
  const { slug } = await params;
  const classItem = getClassBySlug('flare-2026', slug);

  if (!classItem) return { title: 'Class Not Found' };

  return {
    title: `${classItem.title} - FLARE 2026`,
    description: classItem.content?.substring(0, 160) || `${classItem.title} at FLARE 2026`,
  };
}
...
export default async function ClassPage({ params }: ClassPageProps) {
  ...
  const presenters = presenterSlugs
    .map((s) => getPresenterBySlug('flare-2026', s))
    .filter(Boolean);
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {presenters.map((p, i) => (
                  <span key={p!.slug} className="flex items-center gap-2">
                    <Link
                      href={`/events/flare-2026/presenters/${p!.slug}`}
