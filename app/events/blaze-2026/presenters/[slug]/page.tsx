...
export async function generateStaticParams() {
  const presenters = getPresenters('blaze-2026');
...
export async function generateMetadata({ params }: PresenterPageProps): Promise<Metadata> {
  ...
  const presenter = getPresenterBySlug('blaze-2026', slug);
  ...
  return {
    title: `${presenter.name} - BLAZE 2026 Presenter`,
    description: presenter.content?.substring(0, 160) || `${presenter.name} is presenting at BLAZE 2026`,
  };
}
      ...
      <div className="relative z-10 border-b border-fire-dark">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-2 text-sm text-gray-400">
            ...
            <span>/</span>
            <Link href="/events/blaze-2026/presenters" className="hover:text-fire-orange transition-colors">Presenters</Link>
            <span>/</span>
            <span className="text-fire-orange">{presenter.name}</span>
          </div>
        </div>
      </div>
