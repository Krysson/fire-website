...
export default async function ClassPage({ params }: ClassPageProps) {
  ...
  const presenterSlugs = Array.isArray(classItem.presenter)
    ...
    : classItem.presenter
      ? [classItem.presenter]
      : []
  const presenters = presenterSlugs.map(s => getPresenterBySlug('blaze-2026', s)).filter(Boolean)
        ...
        <div className='max-w-3xl mx-auto'>
          ...
          <div className='mb-10'>
            ...
            <div className='flex flex-wrap items-center gap-3 mb-6'>
              <span
                className={`px-3 py-1 rounded-full border font-medium text-sm ${levelBadgeClasses}`}>
                {classItem.level}
              </span>
              {classItem.duration && (
