...
export const metadata: Metadata = {
  title: 'BLAZE 2026 Schedule',
  description:
    'View the full schedule for BLAZE 2026, including classes, workshops, and special events throughout the weekend.',
  openGraph: {
    title: 'BLAZE 2026 Schedule',
    description: 'Full weekend schedule for BLAZE 2026 rope bondage event in Orlando, FL.',
    type: 'website'
  }
}

export default function BlazeSchedulePage() {
  const eventData = getEventData('blaze-2026')
  const schedule = getSchedule('blaze-2026')
  const presenterNames = Object.fromEntries(getPresenters('blaze-2026').map(p => [p.slug, p.name]))
  const classLevels = Object.fromEntries(
    getClasses('blaze-2026')
